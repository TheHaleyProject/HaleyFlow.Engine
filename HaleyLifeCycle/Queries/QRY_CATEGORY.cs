...
namespace Haley.Internal {
    internal static class QRY_CATEGORY {
        public const string INSERT = $@"INSERT IGNORE INTO category (display_name) VALUES ({DISPLAY_NAME}); SELECT id, display_name, name FROM category WHERE name = lower(trim({DISPLAY_NAME})) LIMIT 1;";
        public const string GET_ALL = $@"SELECT id, display_name, name FROM category ORDER BY display_name;";
        public const string GET_BY_NAME = $@"SELECT id, display_name, name FROM category WHERE name = lower(trim({NAME})) LIMIT 1;";
