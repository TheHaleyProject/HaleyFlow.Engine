...
namespace Haley.Services {
    public partial class LifeCycleStoreMaria {
        ...
        public async Task<IFeedback<bool>> Exists(WorkFlowEntity entity, LifeCycleKey key) {
        ...
        public Task<IFeedback<bool>> Delete(WorkFlowEntity entity, LifeCycleKey key) {
            var (sql, args) = BuildDelete(entity, key);
            return _agw.NonQueryAsync(_key, sql, args);
        }

        public Task<IFeedback<Dictionary<string, object>>> Get(WorkFlowEntity entity, LifeCycleKey key) {
            var (sql, args) = BuildGet(entity, key);
            return _agw.ReadSingleAsync(_key, sql, args);
        }

        public Task<IFeedback<List<Dictionary<string, object>>>> List(WorkFlowEntity entity, LifeCycleKey? scope = null, int skip = 0, int limit = 200) {
