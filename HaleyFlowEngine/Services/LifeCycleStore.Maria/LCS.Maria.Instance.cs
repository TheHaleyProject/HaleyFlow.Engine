...
namespace Haley.Services {
    public partial class LifeCycleStoreMaria {
        ...
        public async Task<IFeedback<Dictionary<string, object>>> UpsertInstance(int defVersion, int currentState, int? lastEvent, string externalRef, WorkFlowInstanceFlag flags) {
        ...
        public Task<IFeedback<bool>> UpdateInstanceState(LifeCycleKey key, int newState, int lastEvent, WorkFlowInstanceFlag flags) =>
            key.Type == WorkFlowEntityKeyType.Guid
        ...
        public Task<IFeedback<List<Dictionary<string, object>>>> GetTransitionLogList(LifeCycleKey key, int skip = 0, int limit = 200) {
            var sql = key.Type switch {
                WorkFlowEntityKeyType.Id => QRY_TRANSITION_LOG.GET_BY_INSTANCE,
                WorkFlowEntityKeyType.Composite when key.keys[0] is object[] a && a.Length >= 2 && (a[0] is DateTime || a[1] is DateTime) => QRY_TRANSITION_LOG.GET_BY_DATE_RANGE,
                WorkFlowEntityKeyType.Composite when key.keys[0] is object[] b && b.Length >= 2 => QRY_TRANSITION_LOG.GET_BY_STATE_CHANGE,
            ...
            return key.Type switch {
                WorkFlowEntityKeyType.Id => _agw.ReadAsync(_key, sql, (INSTANCE_ID, Convert.ToInt64(key.keys[0])), (SKIP, skip), (LIMIT, limit)),
                WorkFlowEntityKeyType.Composite when key.keys[0] is object[] a && a.Length >= 2 && (a[0] is DateTime || a[1] is DateTime) => _agw.ReadAsync(_key, sql, (CREATED, a[0]), (MODIFIED, a[1]), (SKIP, skip), (LIMIT, limit)),
                WorkFlowEntityKeyType.Composite when key.keys[0] is object[] b && b.Length >= 2 => _agw.ReadAsync(_key, sql, (FROM_STATE, Convert.ToInt32(b[0])), (TO_STATE, Convert.ToInt32(b[1])), (SKIP, skip), (LIMIT, limit)),
                _ => throw new NotSupportedException("Invalid TransitionLog_List filter.")
            };
        }
