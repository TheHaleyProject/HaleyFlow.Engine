...
namespace Haley.Internal {
    internal sealed class MariaAckDispatchDAL : MariaDALBase, IAckDispatchDAL {
     ...
     => Db.RowsAsync(QRY_ACK_DISPATCH.LIST_DUE_LC_PAGED, load, (CONSUMER_ID, consumer), (ACK_STATUS, status), (TTL_SECONDS, ttlSeconds), (SKIP, skip), (TAKE, take));
        ...
        public Task<DbRows> ListDueCompletePagedAsync(long consumer, int status, int ttlSeconds, int skip, int take, DbExecutionLoad load = default)
            => Db.RowsAsync(QRY_ACK_DISPATCH.LIST_DUE_COMPLETE_PAGED, load, (CONSUMER_ID, consumer), (ACK_STATUS, status), (TTL_SECONDS, ttlSeconds), (SKIP, skip), (TAKE, take));
        ...
        public Task<int?> CountDueCompleteAsync(int status, DbExecutionLoad load = default)
            => Db.ScalarAsync<int?>(QRY_ACK_DISPATCH.COUNT_DUE_COMPLETE, load,(ACK_STATUS, status));
