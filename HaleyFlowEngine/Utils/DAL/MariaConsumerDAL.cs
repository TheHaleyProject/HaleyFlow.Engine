...
namespace Haley.Internal {
    internal sealed class MariaConsumerDAL : MariaDALBase, IConsumerDAL {
        ...
        public async Task<IReadOnlyList<int>> ListAliveEnvCodesAsync(int ttlSeconds, DbExecutionLoad load = default) {
            var rows = await Db.RowsAsync(QRY_CONSUMER.LIST_ALIVE_ENV_CODES, load, (TTL_SECONDS, ttlSeconds));
            return rows.Select(r => (int)r[KEY_ENV_CODE]).ToArray();
        }
