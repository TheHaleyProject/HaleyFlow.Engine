...
namespace Haley.Internal {
    ...
    internal class QRY_ACK_DISPATCH {
        ...
        public const string LIST_PENDING_LC_READY_PAGED = $@"SELECT a.id AS ack_id, a.guid AS ack_guid, a.created AS ack_created, ac.id AS ack_consumer_id, ac.consumer, ac.status, ac.last_retry, ac.retry_count, ac.created AS consumer_created, ac.modified AS consumer_modified, l.id AS lc_id, l.instance_id, l.from_state, l.to_state, l.event, l.created AS lc_created, i.guid AS instance_guid, i.external_ref, d.actor, d.payload FROM lc_ack la JOIN ack a ON a.id = la.ack_id JOIN ack_consumer ac ON ac.ack_id = a.id JOIN lifecycle l ON l.id = la.lc_id JOIN instance i ON i.id = l.instance_id LEFT JOIN lc_data d ON d.lc_id = l.id WHERE ac.status = {ACK_STATUS} AND ac.last_retry < {OLDER_THAN} ORDER BY ac.last_retry ASC, ac.id ASC LIMIT {TAKE} OFFSET {SKIP};";

        public const string LIST_PENDING_LC_READY_BY_CONSUMER = $@"SELECT a.id AS ack_id, a.guid AS ack_guid, a.created AS ack_created, ac.id AS ack_consumer_id, ac.consumer, ac.status, ac.last_retry, ac.retry_count, ac.created AS consumer_created, ac.modified AS consumer_modified, l.id AS lc_id, l.instance_id, l.from_state, l.to_state, l.event, l.created AS lc_created, i.guid AS instance_guid, i.external_ref, d.actor, d.payload FROM lc_ack la JOIN ack a ON a.id = la.ack_id JOIN ack_consumer ac ON ac.ack_id = a.id JOIN lifecycle l ON l.id = la.lc_id JOIN instance i ON i.id = l.instance_id LEFT JOIN lc_data d ON d.lc_id = l.id WHERE ac.consumer = {CONSUMER_ID} AND ac.status = {ACK_STATUS} AND ac.last_retry < {OLDER_THAN} ORDER BY ac.last_retry ASC, ac.id ASC;";
        ...
        public const string LIST_PENDING_HOOK_READY_PAGED = $@"SELECT a.id AS ack_id, a.guid AS ack_guid, a.created AS ack_created, ac.id AS ack_consumer_id, ac.consumer, ac.status, ac.last_retry, ac.retry_count, ac.created AS consumer_created, ac.modified AS consumer_modified, h.id AS hook_id, h.instance_id, h.state_id, h.via_event, h.on_entry, h.route, h.created AS hook_created, i.guid AS instance_guid, i.external_ref FROM hook_ack ha JOIN ack a ON a.id = ha.ack_id JOIN ack_consumer ac ON ac.ack_id = a.id JOIN hook h ON h.id = ha.hook_id JOIN instance i ON i.id = h.instance_id WHERE ac.status = {ACK_STATUS} AND ac.last_retry < {OLDER_THAN} ORDER BY ac.last_retry ASC, ac.id ASC LIMIT {TAKE} OFFSET {SKIP};";

        public const string LIST_PENDING_HOOK_READY_BY_CONSUMER = $@"SELECT a.id AS ack_id, a.guid AS ack_guid, a.created AS ack_created, ac.id AS ack_consumer_id, ac.consumer, ac.status, ac.last_retry, ac.retry_count, ac.created AS consumer_created, ac.modified AS consumer_modified, h.id AS hook_id, h.instance_id, h.state_id, h.via_event, h.on_entry, h.route, h.created AS hook_created, i.guid AS instance_guid, i.external_ref FROM hook_ack ha JOIN ack a ON a.id = ha.ack_id JOIN ack_consumer ac ON ac.ack_id = a.id JOIN hook h ON h.id = ha.hook_id JOIN instance i ON i.id = h.instance_id WHERE ac.consumer = {CONSUMER_ID} AND ac.status = {ACK_STATUS} AND ac.last_retry < {OLDER_THAN} ORDER BY ac.last_retry ASC, ac.id ASC;";
