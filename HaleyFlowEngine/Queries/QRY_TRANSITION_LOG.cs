...
namespace Haley.Internal {
    internal class QRY_TRANSITION_LOG {
        public const string INSERT = $"INSERT INTO transition_log (instance_id, from_state, to_state, event) VALUES ({INSTANCE_ID}, {FROM_STATE}, {TO_STATE}, {EVENT}); SELECT LAST_INSERT_ID();";
        public const string GET_BY_ID = $"SELECT tl.*, td.actor, td.metadata FROM transition_log tl LEFT JOIN transition_data td ON td.transition_log = tl.id WHERE tl.id = {ID} LIMIT 1;";
        ...
        public const string EXISTS_LOG_BY_ID =$@"SELECT 1 FROM transition_log WHERE id = {ID} LIMIT 1;";
    }
}
