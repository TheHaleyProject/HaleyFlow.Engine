...
namespace Haley.Internal {
    internal class QRY_DEFVERSION {
        ...
        public const string GET_BY_GUID = $@"SELECT * FROM def_version WHERE guid = lower(trim({GUID})) LIMIT 1;";
        public const string GET_LATEST_BY_PARENT = $@"SELECT * FROM def_version WHERE parent = {PARENT_ID} ORDER BY version DESC LIMIT 1;";

        public const string LIST_BY_PARENT = $@"SELECT * FROM def_version WHERE parent = {PARENT_ID} ORDER BY version DESC;";

        public const string INSERT = $@"INSERT INTO def_version (parent, version, data) VALUES ({PARENT_ID}, {VERSION}, {DATA}); SELECT LAST_INSERT_ID() AS id;";
        ...
        public const string GET_LATEST_BY_ENV_CODE_AND_DEF_NAME = $@"SELECT dv.* FROM def_version dv JOIN definition d ON d.id = dv.parent JOIN environment e ON e.id = d.env WHERE e.code = {CODE} AND d.name = lower(trim({NAME})) ORDER BY dv.version DESC LIMIT 1;";
        ...
        public const string GET_LATEST_BY_ENV_NAME_AND_DEF_NAME = $@"SELECT dv.* FROM def_version dv JOIN definition d ON d.id = dv.parent JOIN environment e ON e.id = d.env WHERE e.name = lower(trim({ENV_NAME})) AND d.name = lower(trim({DEF_NAME})) ORDER BY dv.version DESC LIMIT 1;";
