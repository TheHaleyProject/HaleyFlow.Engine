...
namespace Haley.Services {
    public partial class LifeCycleStateMariaDB {
        ...
        public Task<IFeedback<Dictionary<string, object>>> GetFinalState(int defVersion) => _agw.ReadSingleAsync(_key, QRY_STATE.GET_FINAL, (DEF_VERSION, defVersion));
        public Task<IFeedback<bool>> UpdateStateFlags(int stateId, LifeCycleStateFlag newFlags) => _agw.NonQueryAsync(_key, QRY_STATE.UPDATE_FLAGS, (FLAGS, (int)newFlags), (ID, stateId));
        public Task<IFeedback<bool>> DeleteState(int stateId) => _agw.NonQueryAsync(_key, QRY_STATE.DELETE, (ID, stateId));
    }
}
